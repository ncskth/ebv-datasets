...
def triangle_improved(r, p, device, thickness=3):
    ...
    outer_full_1 = torch.distributions.Bernoulli(probs=1).sample((r, r)).to(device).bool()
    ...
    outer_full_2 = torch.distributions.Bernoulli(probs=1).sample((r_2, r_2)).to(device).bool()
    ...
    outer_2_padded = torch.nn.functional.pad(outer_full_2, (int(round(1.5*size_padded)), int(round(0.5*size_padded)), size_padded_1, size_padded_2), "constant", 0)
